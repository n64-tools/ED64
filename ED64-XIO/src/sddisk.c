...
u8 sd_disk_initialize() {
  ...
  if ((sd_resp_buff[1] & 64) && disk_card_type != 0)
    disk_card_type |= SD_HC;
...
u8 sd_disk_read_response(u8 cmd) {
  ...
  while ((sd_resp_buff[0] & 0xC0) !=
         0) { /* wait for resp begin. first two bits should be zeros */
    sd_resp_buff[0] = ed64_sdio_command_read();
...
u8 sd_disk_open_read(u32 saddr) {
  ...
  u8 resp;
  if (disk_mode == DISK_MODE_RD && saddr == disk_cur_addr)
    return 0;

  sd_disk_close_rw();
  disk_cur_addr = saddr;
  if ((disk_card_type & SD_HC) == 0)
    saddr *= 512;
...
u8 sd_disk_open_write(u32 saddr) {
  ...
  u8 resp;
  if (disk_mode == DISK_MODE_WR && saddr == disk_cur_addr)
    return 0;
